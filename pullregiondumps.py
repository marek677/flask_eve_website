...
from components.sde import evejumps
...
def doRegionCalc(region_name):
	...
	with open('components/region/dumps/%s_orders.json'%region_name.replace(" ","_"), 'wb') as outfile:
...
esi = ESI()
em = ESIMarket(esi,sd)
print "itemids in game: ",len(map(lambda x: x, sd.Item ))
jf = evejumps.JumpFinder()
print "[%s] All components Loaded..." % datetime.datetime.now()
#print sd.getItem(46217)
#distance: 30000142 - jita, 30004280 - god knows where in aridia
